...
1. **工具白名单是权限边界。** 子代理没有 `Agent` 工具（不让它再套娃），也没有删除类危险操作的空间；内置 `grep`/`find` 同样排除在外，搜索统一走 fff 扩展，与全局搜索纪律一致（见 [09-agents-md.md](09-agents-md.md)）；正文里又加了一条“禁止 rm -rf / git push 等破坏性操作”的双保险。
