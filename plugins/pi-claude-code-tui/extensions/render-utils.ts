...
/**
 * LOCAL MODIFICATION (keep when updating): pad-only helper, never truncates.
 * Used by applyRoundedEditorBorders so the box cannot clip editor rows.
 * Callers must guarantee the text's visible width ≤ `width` (pi's
 * Editor.render output satisfies this).
 */
export function padOnly(text: string, width: number): string {
...
/**
 * LOCAL MODIFICATION: no longer applied — cursor restyle was removed from the
 * editor render (pi's default reverse-video cursor is kept). Dead code, kept
 * for reference when diffing upstream.
 *
...
/**
 * LOCAL MODIFICATION (keep when updating): upstream drew a half-open box
 * (rounded top/bottom only, no sides, all rows truncated to width).
 * Local version draws a fully closed box with │ sides and is pure
 * decoration: replaces the two border rows, pads short rows, adds sides —
 * it never truncates.
 *
 * Precondition: every input line must have visible width ≤ width - 2.
 * The caller satisfies this by rendering the editor at width - 2, so the
 * editor's own wrap point, padding, and cursor all stay native.
 */
export function applyRoundedEditorBorders(
	...
	return result.map((line, i) => {
		// Rows inside the box get │ sides; autocomplete rows below the bottom
		// border are only padded to full width.
		if (i <= 0 || i >= bottomIdx) return padOnly(line, width);
		return `${color("│")}${padOnly(line, innerWidth)}${color("│")}`;
	});
}
